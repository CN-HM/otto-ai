...
public partial class AiAdminDataSeeder : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task SeedSystemMcpTools()
    {
        ...
        var tools = new[]
        {
            ...
            new { Code = McpSystemToolCodes.TodoList,              Name = "查询任务",     Category = McpToolCategory.Task,         Description = "查询当前待处理或即将到期的任务和提醒列表，用于检查有哪些未完成的事项。",            ParamSchema = "{\"status\":\"string\",\"signal_type\":\"string\",\"limit\":\"integer\"}" },
            new { Code = McpSystemToolCodes.TodoCreate,            Name = "创建待办",     Category = McpToolCategory.Task,         Description = "当用户要求设置闹钟、创建提醒、安排定时任务、记录待办事项时使用。支持设置scheduled_at字段来指定触发时间。",                ParamSchema = "{\"title\":\"string\",\"content\":\"string\",\"signal_type\":\"string\",\"severity\":\"string\",\"scheduled_at\":\"string\"}" },
            new { Code = McpSystemToolCodes.TodoExecute,           Name = "执行任务",     Category = McpToolCategory.Task,         Description = "执行任务的提醒通知，通过短信或邮件发送提醒。需要指定signal_id和通知渠道(sms/email)。",       ParamSchema = "{\"signal_id\":\"string\",\"notification_channel\":\"string\",\"notification_message\":\"string\"}" },
